...
class Order2Spec extends BaseCompilerSpec {
  ...
  val order: u.Tree => Map[u.TermSymbol, Short] = tree =>
  ...
  "driver with control flow" in {
    ...
    for (field <- Seq("cRe", "cIm", "i", "zIm", "zRe", "cond", "rSq", "iSq", "res"))
      withClue(s"O($field):")(O(S(field)) shouldBe 0)
  }
  ...
  "simple driver with a comprehension" in {
    ...
    for (field <- Seq("x", "y", "z1"))
      withClue(s"O($field):")(O(S(field)) shouldBe 1)
    for (field <- Seq("u", "z2"))
      withClue(s"O($field):")(O(S(field)) shouldBe 2)
    for (field <- Seq("v", "w"))
      withClue(s"O($field):")(O(S(field)) shouldBe 3)
  }
}
