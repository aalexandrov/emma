...
private[backend] trait Context extends Common {
  ...
  object Context {
     ...
     */
    def bCtxGraph(tree: u.Tree): quiver.Graph[u.TermSymbol, BCtx, Boolean] = {
     ...
     * - edges are labeled with a boolean flag inidcating use in a data-parallel operator.
     */
    def bCtxGraph(G: FlowGraph[u.TermSymbol]): quiver.Graph[u.TermSymbol, BCtx, Boolean] = {
      val bCtx = bCtxMap(G)
    ...
    def bCtxMap(G: FlowGraph[u.TermSymbol]): Map[u.TermSymbol, BCtx] = {
      ...
      for (s <- G.data.nodes) {
        bCtxt.get(s) match {
          case None =>
            warning(s"Cannot determine binding context of $s", s.pos)
          case Some(BCtx.Ambiguous) =>
            warning(s"Ambiguous binding context of $s", s.pos)
