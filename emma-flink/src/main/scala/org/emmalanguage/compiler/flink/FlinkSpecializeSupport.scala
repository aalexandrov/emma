...
private[compiler] trait FlinkSpecializeSupport {
  self: FlinkCompiler =>
  ...
  object FlinkSpecializeSupport {

    /** Introduces Flink's native iterations whenever possible. */
    lazy val specializeLoops: u.Tree => u.Tree = tree => {
      api.BottomUp.withValUses.transformWith({
        case Attr.syn(t@core.Let(
          ...
          val fRhs = core.Lambda(Seq(xs), core.Let(
            splitDataflow(vals3, api.Tree.refs(xs2).toSet[u.Symbol]).get,
            Seq(),
            xs2
          ))
