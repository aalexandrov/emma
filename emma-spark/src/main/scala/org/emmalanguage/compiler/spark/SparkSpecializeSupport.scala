  ...
  object SparkSpecializeSupport {
    ...
    lazy val specializeLambda: u.Tree => u.Tree = {
      case lambda@core.Lambda(_, Seq(core.ParDef(p, _)), core.Let(vals, Seq(), core.Ref(r))) =>
        ...
        def isTupleApply(x: u.TermSymbol): Boolean = valOf.get(x) match {
        ...
        if (valOf.contains(r)) {
          val mapSym = Map(
            ...
            val W =
              if (x == r && isTupleApply(x)) api.Type.kind1[Seq](Exp.Column)
              else Exp.Column
          ...
          val (vals2, expr1) = if (isTupleApply(r)) {
            val vs2 = Seq.empty[Option[u.ValDef]]
            val ex2 = Some(core.Ref(mapSym(r)))
            ...
            val ex2 = Some(core.Ref(lhs))

            (vs2, ex2)
          }

          if ((expr1 +: (vals1 ++ vals2)).exists(_.isEmpty)) lambda
          else core.Lambda(Seq(mapSym(p)), core.Let((vals1 ++ vals2).flatten, Seq(), expr1.get))
        } else lambda
